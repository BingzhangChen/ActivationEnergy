...
mtext(expression(paste(bold(A)*') '*italic(E[app])*' & '*italic(E[intra]))), adj=0, cex=1.4)
...
mtext(expression(paste(bold(B)*') '* italic(E[inter]))), adj=0, cex=1.4)
...
mtext(expression(paste(italic(y[mj]))), side = 2, adj = .77,  col = 3)
mtext('0', side = 1, adj = .52, col = 2, line=0.1)
mtext(expression(paste(bold(C)*') '* italic(E[L]))), adj=0, cex=1.4)
dev.off()
