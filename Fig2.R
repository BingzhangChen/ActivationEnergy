...
plot_Einter <- function(oridat, XLAB = '', plot.legend = FALSE){
 ...
 unidat$Col_code <- NA
  ...
  if (plot.legend){
      txt <- paste(ToptC, 'ºC')
      legend('bottomleft', legend=txt, col=jet.colors(Ncol), cex=.6,pch=16)
...
plot_Ea_example(zoodat2, XLAB = expression(paste(italic(x) ) ) ) 
...
plot_Einter(zoodat2, XLAB = expression(paste(italic(bar(x)))), plot.legend = T )
